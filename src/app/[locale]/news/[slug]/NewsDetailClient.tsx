...
export default function NewsDetailClient({ newsData, locale, slug }: NewsDetailClientProps) {
  ...
  const relatedNews = relatedResult?.data
    ?.filter(item => item.id !== newsData.id) // Loại bỏ bài đang xem
    .slice(0, 3) || []; // Lấy 3 bài
        ...
        <TopControl>
          ...
          <ArticleCard>
            ...
            <HeaderContent>
              ...
              <MetaInfo>
                {/* HIỂN THỊ DANH MỤC (Requirement 3 from Doc) */}
                {newsData.category && (
                  <span className="category-tag">
                    <RiPriceTag3Line /> {newsData.category.name}
                  </span>
                )}
